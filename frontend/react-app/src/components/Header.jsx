...
class Search extends Component {
  ...
  displaySearchResults(obj) {
    if (!obj.error) {
	    let { category, title } = obj
	    this.setState({
	      redirect: `/category/${category}/${title}`
	    })
    }
  ...
  async getPosts(param) {
    ...
    var response =  await fetch(`https://davinci.pythonanywhere.com/posts/${param}`, options)
    if (!response.error) {
      var data = await response.json()
      console.log(data);
      if (data.title === "") {
        console.log('Not Found');
        this.displaySearchResults({error: 'No results'})
        return
      }
        ...
        <div className="search">
          <div className="search-box">
            ...
            <span
             ...
             onClick={
              () => {
                this.getPosts(this.state.input)
              }
             }
             >
              <svg
